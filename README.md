...
- 要求：
  - 后缀必须为 `.md`（Markdown文件）
  - 文件名仅使用英文、数字、横杠`-`，避免中文、空格、特殊符号，防止编码异常
  - 日期格式为 `年月日`（8位数字），用于快速识别文章发布时间
...
pinned: true          # 是否置顶（true=置顶，false=不置顶，非必填，默认false）
...
|------------|----------------|----------------|
| AI & 技术  | ai-tech        | article/ai-tech/ |
| 财经学习   | finance        | article/finance/ |
| 生活旅行   | life-travel    | article/life-travel/ |
| 游戏日志   | game-log       | article/game-log/ |
...
示例：AI & 技术的文章放入 article/ai-tech/ 目录

#### 3.提交并推送文章到远程仓库
```bash
# 1. 将新文章加入 Git 追踪
git add article/ai-tech/你的文章文件名.md
...
### 2. 新增对应分类文件夹
在 article/ 目录下，新建和 data-category 同名的文件夹，示例：新增 article/book-note/ 文件夹，用于存放该分类的文章。

### 3. 更新文档与配置
更新本 README 中的「分类可选值」表格，补充新增的分类信息
...
排查 4：确认文章文件放入了对应分类的文件夹，且后缀为 .md
